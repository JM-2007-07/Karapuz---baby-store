...
export default function About() {
    ...
    const advantages = [
        {icon:<FavoriteBorderIcon />,title:t('about.advantages.care.title'),text:t('about.advantages.care.text')},
        {icon:<WorkspacePremiumOutlinedIcon />,title:t('about.advantages.quality.title'),text:t('about.advantages.quality.text')},
        {icon:<LocalShippingOutlinedIcon />,title:t('about.advantages.delivery.title'),text:t('about.advantages.delivery.text')},
        {icon:<SupportAgentOutlinedIcon />,title:t('about.advantages.support.title'),text:t('about.advantages.support.text')}
    ]
    const stats = [
        {number:'10+',text:t('about.stats.years')},
        {number:'25 000+',text:t('about.stats.customers')},
        {number:'5 000+',text:t('about.stats.products')},
        {number:'100%',text:t('about.stats.care')}
    ]
    const aboutPoints = [
        t('about.points.quality'),
        t('about.points.brands'),
        t('about.points.attention'),
        ...
        <Box sx={{width:'100%',overflow:'hidden',pt:{xs:'105px',md:'150px'},pb:{xs:'50px',md:'90px'}}}>
                    ...
                    <Box data-aos="fade-left" sx={{position:'relative',zIndex:2,display:'flex',justifyContent:'center',alignItems:'center',minHeight:{xs:'260px',md:'400px'}}}>
                        <Box sx={{width:{xs:'220px',md:'340px'},height:{xs:'220px',md:'340px'},borderRadius:'50%',bgcolor:'#FFFFFF',display:'flex',justifyContent:'center',alignItems:'center',boxShadow:'0 25px 70px rgba(68,107,128,0.12)',position:'relative'}}>
                            ...
                            <Box sx={{position:'absolute',top:{xs:'0px',md:'15px'},right:{xs:'-25px',md:'-55px'},bgcolor:'#FFFFFF',px:{xs:'12px',md:'18px'},py:{xs:'10px',md:'14px'},borderRadius:'18px',boxShadow:'0 15px 35px rgba(68,107,128,0.12)'}}>
                                <Typography sx={{color:'#7FC9F0',fontSize:{xs:'20px',md:'28px'},fontWeight:700}}>10+</Typography>
                                <Typography sx={{color:'rgba(68,107,128,0.65)',fontSize:{xs:'10px',md:'12px'}}}>{t('about.hero.years')}</Typography>
                            </Box>
                            <Box sx={{position:'absolute',bottom:{xs:'5px',md:'15px'},left:{xs:'-25px',md:'-55px'},bgcolor:'#FFFFFF',px:{xs:'12px',md:'18px'},py:{xs:'10px',md:'14px'},borderRadius:'18px',boxShadow:'0 15px 35px rgba(68,107,128,0.12)'}}>
                                <Typography sx={{color:'#446B80',fontSize:{xs:'20px',md:'28px'},fontWeight:700}}>25K+</Typography>
                                <Typography sx={{color:'rgba(68,107,128,0.65)',fontSize:{xs:'10px',md:'12px'}}}>{t('about.hero.customers')}</Typography>
                            </Box>
                        </Box>
                    </Box>
                </Box>
                ...
                <Box sx={{display:'grid',gridTemplateColumns:{xs:'1fr',md:'0.8fr 1.2fr'},gap:{xs:'40px',md:'90px'},alignItems:'center'}}>
                    <Box data-aos="fade-right" sx={{position:'relative',minHeight:{xs:'300px',md:'430px'},borderRadius:'30px',bgcolor:'#F2FAFD',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
                        ...
                        <Box sx={{position:'absolute',bottom:'25px',right:'25px',bgcolor:'#446B80',color:'#FFFFFF',p:'18px',borderRadius:'20px',maxWidth:'180px'}}>
                            <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',fontSize:'18px',fontWeight:700,mb:'4px'}}>{t('about.mission.title')}</Typography>
                            <Typography sx={{fontSize:'12px',opacity:0.8,lineHeight:1.5}}>{t('about.mission.text')}</Typography>
                        </Box>
                    </Box>
                    <Box data-aos="fade-left">
                        <Typography sx={{color:'#7FC9F0',fontSize:'13px',fontWeight:700,textTransform:'uppercase',letterSpacing:'1.5px',mb:'12px'}}>{t('about.story.label')}</Typography>
                        <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',color:'#446B80',fontSize:{xs:'30px',md:'42px'},fontWeight:700,lineHeight:1.2,mb:'20px'}}>{t('about.story.title')}</Typography>
                        <Typography sx={{color:'rgba(68,107,128,0.72)',fontSize:'15px',lineHeight:1.8,mb:'18px'}}>{t('about.story.text1')}</Typography>
                        <Typography sx={{color:'rgba(68,107,128,0.72)',fontSize:'15px',lineHeight:1.8,mb:'25px'}}>{t('about.story.text2')}</Typography>
                        <Box sx={{display:'flex',flexDirection:'column',gap:'12px'}}>
                            {aboutPoints.map((item,index) => (
                                <Box key={index} sx={{display:'flex',alignItems:'center',gap:'10px'}}>
                                    ...
                                    <Typography sx={{color:'#446B80',fontSize:'14px',fontWeight:500}}>{item}</Typography>
                                </Box>
                            ))}
                        </Box>
                    </Box>
                ...
                <Box sx={{maxWidth:'1200px',mx:'auto',px:{xs:'15px',md:'25px'},display:'grid',gridTemplateColumns:{xs:'1fr 1fr',md:'repeat(4,1fr)'},gap:{xs:'35px',md:'20px'}}}>
                    {stats.map((item,index) => (
                        <Box key={index} data-aos="zoom-in" data-aos-delay={index * 100} sx={{textAlign:'center'}}>
                            <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',color:'#FFFFFF',fontSize:{xs:'30px',md:'42px'},fontWeight:700}}>{item.number}</Typography>
                            <Typography sx={{color:'rgba(255,255,255,0.7)',fontSize:{xs:'12px',md:'14px'},mt:'5px'}}>{item.text}</Typography>
                        </Box>
                    ))}
                </Box>
            </Box>
            <Box sx={{width:'100%',maxWidth:'1200px',mx:'auto',px:{xs:'15px',md:'25px'},mb:{xs:'70px',md:'110px'}}}>
                <Box sx={{textAlign:'center',maxWidth:'650px',mx:'auto',mb:{xs:'35px',md:'55px'}}}>
                    <Typography sx={{color:'#7FC9F0',fontSize:'13px',fontWeight:700,textTransform:'uppercase',letterSpacing:'1.5px',mb:'12px'}}>{t('about.advantages.label')}</Typography>
                    <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',color:'#446B80',fontSize:{xs:'30px',md:'42px'},fontWeight:700,mb:'15px'}}>{t('about.advantages.title')}</Typography>
                    <Typography sx={{color:'rgba(68,107,128,0.65)',fontSize:'14px',lineHeight:1.7}}>{t('about.advantages.description')}</Typography>
                </Box>
                <Box sx={{display:'grid',gridTemplateColumns:{xs:'1fr',sm:'1fr 1fr',md:'repeat(4,1fr)'},gap:'20px'}}>
                    {advantages.map((item,index) => (
                        <Box key={index} data-aos="fade-up" data-aos-delay={index * 100} sx={{bgcolor:'#FFFFFF',border:'1px solid rgba(127,201,240,0.18)',borderRadius:'22px',p:'25px',minHeight:'250px',transition:'0.3s','&:hover':{transform:'translateY(-8px)',boxShadow:'0 20px 45px rgba(68,107,128,0.12)',borderColor:'rgba(127,201,240,0.5)'}}}>
                            <Box sx={{width:'58px',height:'58px',borderRadius:'18px',bgcolor:'rgba(127,201,240,0.13)',color:'#7FC9F0',display:'flex',alignItems:'center',justifyContent:'center',mb:'20px','& svg':{fontSize:'28px'}}}>{item.icon}</Box>
                            <Typography sx={{color:'#446B80',fontSize:'17px',fontWeight:700,mb:'10px'}}>{item.title}</Typography>
                            <Typography sx={{color:'rgba(68,107,128,0.65)',fontSize:'13px',lineHeight:1.7}}>{item.text}</Typography>
                        </Box>
                    ))}
                </Box>
            </Box>
            <Box sx={{width:'100%',maxWidth:'1200px',mx:'auto',px:{xs:'15px',md:'25px'}}}>
                <Box data-aos="zoom-in" sx={{position:'relative',overflow:'hidden',borderRadius:{xs:'25px',md:'35px'},bgcolor:'#7FC9F0',p:{xs:'40px 20px',md:'65px'},textAlign:'center'}}>
                    ...
                    <Box sx={{position:'relative',zIndex:2}}>
                        <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',color:'#FFFFFF',fontSize:{xs:'30px',md:'46px'},fontWeight:700,mb:'15px'}}>{t('about.cta.title')}</Typography>
                        <Typography sx={{maxWidth:'600px',mx:'auto',color:'rgba(255,255,255,0.85)',fontSize:{xs:'14px',md:'16px'},lineHeight:1.7,mb:'28px'}}>{t('about.cta.description')}</Typography>
