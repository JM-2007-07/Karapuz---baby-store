...
import { Menu, PersonOutlineOutlined, PlaceOutlined, ShoppingCartOutlined, SearchOutlined, CloseOutlined, FavoriteBorderOutlined, SettingsOutlined, LogoutOutlined, Inventory2Outlined } from '@mui/icons-material'
...
export default function Header() {
    ...
    const catalogItems = [
        {name:'Акции',path:'/sales'},
        {name:'Кроватки',path:'/catalog/cribs'},
        {name:'Коляски',path:'/catalog/strollers'},
        {name:'Автокресла',path:'/catalog/car-seats'},
        {name:'Одежда',path:'/catalog/clothes'},
        {name:'Кормление',path:'/catalog/feeding'},
        {name:'Гигиена и уход',path:'/catalog/hygiene'},
        {name:'Умные игрушки',path:'/catalog/smart-toys'}
    ]
    ...
    const inputStyles = {
        '& .MuiOutlinedInput-root':{
            ...
            bgcolor:'#F8FBFC',
            fontSize:'13px',
            '& fieldset':{borderColor:'rgba(127,201,240,0.2)'},
    ...
    const LoginForm = () => (
        <>
            <TextField fullWidth placeholder="Электронный адрес" type="email" size="small" value={loginEmail} onChange={(e) => {setLoginEmail(e.target.value);setLoginError('')}} sx={{...inputStyles,mb:'13px'}} />
            <TextField fullWidth placeholder="Пароль" type="password" size="small" value={loginPassword} onChange={(e) => {setLoginPassword(e.target.value);setLoginError('')}} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} sx={{...inputStyles,mb:'10px'}} />
            {loginError && <Typography sx={{color:'#E57373',fontSize:'12px',mb:'12px'}}>{loginError}</Typography>}
            <Button onClick={handleLogin} fullWidth sx={{height:'46px',bgcolor:'#7FC9F0',color:'#FFFFFF',borderRadius:'9px',textTransform:'none',fontSize:'14px',fontWeight:500,'&:hover':{bgcolor:'#6CBBE4'}}}>Войти</Button>
            <Typography sx={{fontSize:'13px',textAlign:'center',color:'#7FC9F0',mt:'16px',cursor:'pointer'}}>Забыли пароль?</Typography>
        </>
    )
            ...
            <Box component="header" sx={{position:'fixed',top:0,left:0,width:'100%',zIndex:1400,bgcolor:'rgba(255,255,255,0.72)',backdropFilter:'blur(18px)',borderBottom:'1px solid rgba(127,201,240,0.12)',transition:'0.3s'}}>
                <Box sx={{maxWidth:'1500px',margin:'auto',minHeight:'138px',px:{xs:'15px',sm:'25px',md:'35px'},display:{xs:'none',md:'flex'},flexDirection:'column',justifyContent:'center',gap:'13px'}}>
                    <Box sx={{width:'100%',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'30px'}}>
                        ...
                        <TextField placeholder="Я хочу купить..." size="small" sx={{width:'100%',maxWidth:'480px','& .MuiOutlinedInput-root':{height:'44px',borderRadius:'7px',bgcolor:'rgba(255,255,255,0.35)',fontSize:'14px',color:'#446B80','& fieldset':{borderColor:'rgba(127,201,240,0.15)'},'&:hover fieldset':{borderColor:'#7FC9F0'},'&.Mui-focused fieldset':{borderColor:'#7FC9F0'}},'& input::placeholder':{color:'#8BA0AB',opacity:1}}} InputProps={{startAdornment:<InputAdornment position="start"><SearchOutlined sx={{color:'#7FC9F0',fontSize:'21px'}} /></InputAdornment>}} />
                        <Box sx={{display:'flex',alignItems:'center',gap:'28px',flexShrink:0}}>
                            ...
                            <Button onClick={() => navigate('/cart')} sx={{display:'flex',alignItems:'center',gap:'7px',fontSize:'14px',color:'#446B80',textTransform:'none',whiteSpace:'nowrap',p:0,minWidth:0}}>
                                <Badge badgeContent={cartCount} color="error">
                                    <ShoppingCartOutlined sx={{color:'#7FC9F0',width:'21px',height:'21px'}} />
                                </Badge>
                                Корзина 
                            </Button>
                        </Box>
                    ...
                    <Box sx={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'25px'}}>
                        <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',width:'155px',fontSize:'10px',lineHeight:'15px',color:'#446B80',flexShrink:0}}>Онлайн гипермаркет<br />товаров для детей</Typography>
                        ...
                        <Box sx={{display:'flex',gap:'4px',alignItems:'center',color:'#446B80',flexShrink:0}}>
                            <PlaceOutlined sx={{width:'19px',height:'19px',color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'13px',whiteSpace:'nowrap'}}>Город: <span style={{color:'#7FC9F0'}}>Москва</span></Typography>
                        </Box>
                    </Box>
                </Box>
    ...
    <Box sx={{width:'1530px',mx:'auto',height:'600px',display:'flex',bgcolor:'#FFFFFF'}}>
        ...
        <Box onClick={() => setCatalogOpen(false)} sx={{flex:1,height:'100%',bgcolor:'#FFFFFF',cursor:'pointer'}} />
            ...
            <Drawer anchor="left" open={menuOpen} onClose={() => setMenuOpen(false)} PaperProps={{sx:{width:{xs:'88%',sm:'360px'},maxWidth:'380px',bgcolor:'#FFFFFF',pt:'8px'}}}>
                <Box sx={{width:'100%',minHeight:'100vh',display:'flex',flexDirection:'column'}}>
                    ...
                    <Box sx={{px:'18px',pt:'8px',pb:'18px'}}>
                        <TextField fullWidth placeholder="Я хочу купить..." size="small" sx={{'& .MuiOutlinedInput-root':{height:'46px',borderRadius:'8px',bgcolor:'#F7FAFC','& fieldset':{borderColor:'rgba(127,201,240,0.2)'},'&.Mui-focused fieldset':{borderColor:'#7FC9F0'}}}} InputProps={{startAdornment:<InputAdornment position="start"><SearchOutlined sx={{color:'#7FC9F0'}} /></InputAdornment>}} />
                    </Box>
                    ...
                    <Box sx={{mt:'auto',px:'18px',py:'20px',borderTop:'1px solid rgba(68,107,128,0.12)'}}>
                        <Box sx={{display:'flex',alignItems:'center',gap:'6px'}}>
                            <PlaceOutlined sx={{fontSize:'20px',color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'14px',color:'#446B80'}}>Город: <span style={{color:'#7FC9F0'}}>Москва</span></Typography>
                        </Box>
                    </Box>
                </Box>
            </Drawer>
            ...
            <Popover open={accountMenuOpen} anchorEl={accountAnchor} onClose={handleAccountClose} anchorOrigin={{vertical:'bottom',horizontal:'right'}} transformOrigin={{vertical:'top',horizontal:'right'}} slotProps={{paper:{sx:{mt:'14px',width:'330px',borderRadius:'16px',overflow:'hidden',boxShadow:'0 15px 45px rgba(68,107,128,0.18)',border:'1px solid rgba(127,201,240,0.15)',bgcolor:'rgba(255,255,255,0.98)',backdropFilter:'blur(20px)'}}}}>
                {!isLoggedIn ? (
                    <Box sx={{p:'22px'}}>
                        <Box sx={{display:'flex',alignItems:'center',justifyContent:'space-between',mb:'22px'}}>
                            <Typography onClick={() => {navigate('/register');handleAccountClose()}} sx={{fontFamily:'"Balsamiq Sans", sans-serif',fontSize:'19px',fontWeight:700,color:'#7FC9F0',cursor:'pointer','&:hover':{textDecoration:'underline'}}}>Регистрация</Typography>
                    ...
                    <Box sx={{py:'12px'}}>
                        <Box sx={{cursor:'pointer',display:'flex',alignItems:'center',gap:'13px',px:'20px',py:'10px',pb:'16px'}}>
                            <Box sx={{width:'38px',height:'38px',borderRadius:'50%',bgcolor:'#EEF8FC',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                                <PersonOutlineOutlined sx={{color:'#7FC9F0',fontSize:'22px'}} />
                            </Box>
                            <Box sx={{minWidth:0}}>
                                <Typography sx={{fontSize:'15px',fontWeight:500,color:'#446B80'}}>{user?.name || 'Пользователь'}</Typography>
                        ...
                        <Box onClick={() => {navigate('/orders');handleAccountClose()}} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'12px',cursor:'pointer','&:hover':{bgcolor:'#F5FAFC'}}}>
                            <Inventory2Outlined sx={{fontSize:'20px',color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Мои заказы</Typography>
                        </Box>
                        <Box onClick={() => {navigate('/favorites');handleAccountClose()}} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'12px',cursor:'pointer','&:hover':{bgcolor:'#F5FAFC'}}}>
                            <FavoriteBorderOutlined sx={{fontSize:'20px',color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Мое избранное</Typography>
                        </Box>
                        <Divider sx={{my:'5px',borderColor:'rgba(68,107,128,0.12)'}} />
                        <Box onClick={() => {navigate('/personal-data');handleAccountClose()}} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'12px',cursor:'pointer','&:hover':{bgcolor:'#F5FAFC'}}}>
                            <SettingsOutlined sx={{fontSize:'20px',color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Настройки личных данных</Typography>
                        </Box>
                        <Box onClick={handleLogout} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'12px',cursor:'pointer','&:hover':{bgcolor:'#FFF5F5'}}}>
                            <LogoutOutlined sx={{fontSize:'20px',color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Выйти</Typography>
                        </Box>
                    </Box>
                )}
            </Popover>
                ...
                {!isLoggedIn ? (
                    <Box sx={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
                        <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center',px:'20px',py:'15px'}}>
                            <Typography onClick={() => {setAccountOpen(false);navigate('/register')}} sx={{fontFamily:'"Balsamiq Sans", sans-serif',fontSize:'19px',fontWeight:700,color:'#7FC9F0',cursor:'pointer'}}>Регистрация</Typography>
                    ...
                    <Box sx={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
                        <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center',px:'20px',py:'18px'}}>
                            <Typography sx={{fontFamily:'"Balsamiq Sans", sans-serif',fontSize:'19px',fontWeight:700,color:'#446B80'}}>Личный кабинет</Typography>
                            <IconButton onClick={() => setAccountOpen(false)} sx={{color:'#446B80'}}><CloseOutlined /></IconButton>
                        </Box>
                        <Divider sx={{borderColor:'rgba(68,107,128,0.12)'}} />
                        <Box onClick={() => {navigate('/personal-data');setAccountOpen(false)}} sx={{display:'flex',alignItems:'center',gap:'13px',px:'20px',py:'22px',cursor:'pointer','&:hover':{bgcolor:'#F5FAFC'}}}>
                            <Box sx={{width:'45px',height:'45px',borderRadius:'50%',bgcolor:'#EEF8FC',display:'flex',alignItems:'center',justifyContent:'center'}}>
                                <PersonOutlineOutlined sx={{color:'#7FC9F0',fontSize:'24px'}} />
                            </Box>
                            <Box>
                                <Typography sx={{fontSize:'16px',fontWeight:600,color:'#446B80'}}>{user?.name || 'Пользователь'}</Typography>
                                <Typography sx={{fontSize:'12px',color:'rgba(68,107,128,0.65)'}}>{user?.email}</Typography>
                            </Box>
                        </Box>
                        <Divider sx={{borderColor:'rgba(68,107,128,0.12)'}} />
                        <Box onClick={() => {navigate('/orders');setAccountOpen(false)}} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'15px',cursor:'pointer'}}>
                            <Inventory2Outlined sx={{color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Мои заказы</Typography>
                        </Box>
                        <Box onClick={() => {navigate('/favorites');setAccountOpen(false)}} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'15px',cursor:'pointer'}}>
                            <FavoriteBorderOutlined sx={{color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Мое избранное</Typography>
                        </Box>
                        <Divider sx={{borderColor:'rgba(68,107,128,0.12)'}} />
                        <Box onClick={() => {navigate('/personal-data');setAccountOpen(false)}} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'15px',cursor:'pointer'}}>
                            <SettingsOutlined sx={{color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Настройки личных данных</Typography>
                        </Box>
                        <Box onClick={handleLogout} sx={{display:'flex',alignItems:'center',gap:'12px',px:'20px',py:'15px',cursor:'pointer'}}>
                            <LogoutOutlined sx={{color:'#7FC9F0'}} />
                            <Typography sx={{fontSize:'15px',color:'#446B80'}}>Выйти</Typography>
                        </Box>
                    </Box>
                )}
